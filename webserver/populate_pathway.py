...
from rolls.models import Pathway  # Sostituisci con il nome della tua app

# Funzione per popolare il modello Gene
def populate_genes():
    # Leggi la lista di geni dal file
    with open('pathway_list.txt', 'r') as f:
        genes = f.readlines()
    ...
    for gene_name in genes:
        pathway, created = Pathway.objects.get_or_create(pathway=gene_name)
        if created:
